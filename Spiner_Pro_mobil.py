...
def main(page: ft.Page):
    ...
    def crear_mensaje(patrones: list, patron: str) -> str:
    ...
    def reed_URL(TARGET, START_URL):
        ...
        for i in range(need_pages):
            ...
            if not items:
                mensajes_txt.value = f"Sin datos en página {page_num}"
                page.update()
                break

            for it in items:
                norm = normalize(it)
                valores = [norm['numero'], norm['startedAt'], orden]
                orden += 1
            ...
            mensajes_txt.value = f"Página {page_num} -> acumuladas: {len(all_rows)}"
            page.update()
            if len(all_rows) >= TARGET:
                break
            time.sleep(0.3)
    ...
    def recopilar_jugadas(e):
        ...
        if not sel or not meta:
            ...
            reed_URL(info.get('max_numeros'), info.get("API"))
...
ft.app(target=main, view=None, host="0.0.0.0", port=8550)