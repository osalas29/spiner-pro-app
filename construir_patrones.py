...
# ==============================
# FUNCIÓN DE BÚSQUEDA DEL PATRÓN (LA CORRECCIÓN)
# Esta función es la que debe llamar desde Sniper_Pro_Version_Luis.py
# ==============================

def obtener_jugada_por_patron_colores(historial_colores_7):
    """
    Busca el patrón de 7 colores más reciente en conf_patrones.json.
    
    Argumento:
        historial_colores_7: Lista o deque de 7 colores (ej: ['R', 'N', 'R', 'R', 'R', 'R', 'R']) 
                             en orden: [MÁS RECIENTE, ..., MÁS ANTIGUO].
                             
    Retorna:
        El objeto patrón encontrado del JSON o None si no hay coincidencia.
    """
    # Verificación de tamaño de historial
    if not historial_colores_7 or len(historial_colores_7) < 7:
        return None

    # 1. Aplicar la CORRECCIÓN: Invertir la secuencia.
    # El historial viene [Reciente, ..., Antiguo]. El JSON lo espera [Antiguo, ..., Reciente].
    historial_invertido = list(reversed(historial_colores_7))
    
    # 2. Convertir la secuencia invertida a la cadena compacta de 7 caracteres.
    patron_actual = "".join(historial_invertido) 
    
    # 3. Cargar el JSON de patrones
    try:
        # Asumiendo que conf_patrones.json está en el mismo directorio
        with open("conf_patrones.json", "r") as f:
            datos_patrones = json.load(f)
    except FileNotFoundError:
        print("Error: No se encontró 'conf_patrones.json'. Asegúrese de que esté en la misma carpeta.")
        return None
    except json.JSONDecodeError:
        print("Error: 'conf_patrones.json' no es un JSON válido.")
        return None
    
    # 4. Buscar la coincidencia en la estrategia "Colores"
    for estrategia in datos_patrones:
        # Nota: El JSON tiene 'estrategia', no 'Patrones' en el primer nivel.
        if estrategia.get("estrategia") == "Colores":
            for patron_def in estrategia.get("Patrones", []):
                # Comparar con el patrón compacto del JSON (ej: "RRRRRRR")
                if patron_def.get("Patron") == patron_actual:
                    # ¡Encontrado! Retornar la jugada completa
                    return patron_def 
    
    return None

def obtener_jugada_por_patron_paridad(historial_paridad_7):
    """
    Busca el patrón de 7 paridades más reciente en conf_patrones.json (Estrategia: Paridad).
    """
    if not historial_paridad_7 or len(historial_paridad_7) < 7:
        return None

    # CORRECCIÓN: Invertir la secuencia. [Reciente, ..., Antiguo] -> [Antiguo, ..., Reciente]
    historial_invertido = list(reversed(historial_paridad_7))
    patron_actual = "".join(historial_invertido) 
    ...
    # Buscar la coincidencia en la estrategia "Paridad"
    for estrategia in datos_patrones:
        if estrategia.get("estrategia") == "Paridad":
            for patron_def in estrategia.get("Patrones", []):
                if patron_def.get("Patron") == patron_actual:
                    return patron_def 
...
def obtener_jugada_por_patron_powerby(historial_powerby_X):
    """
    Busca el patrón más reciente en conf_patrones.json (Estrategia: Power By).
    Argumento: historial_powerby_X es la secuencia del historial de Power By (ej: ['1', '0', '1', ...]).
    """
    # Asumo una longitud mínima de 7 para la búsqueda de patrones por consistencia
    if not historial_powerby_X or len(historial_powerby_X) < 7:
        return None

    # CORRECCIÓN: Invertir la secuencia.
    # El historial viene [Reciente, ..., Antiguo]. El JSON lo espera [Antiguo, ..., Reciente].
    historial_invertido = list(reversed(historial_powerby_X))
    patron_actual = "".join(str(x) for x in historial_invertido) 
    ...
    # Buscar la coincidencia en la estrategia "Power By"
    for estrategia in datos_patrones:
        # El nombre de la estrategia debe coincidir con el JSON
        if estrategia.get("estrategia") == "Power By": 
            for patron_def in estrategia.get("Patrones", []):
                if patron_def.get("Patron") == patron_actual:
                    return patron_def 
...
def obtener_jugada_por_patron_combinado(historial_combinado_X):
    """
    Busca el patrón combinado más reciente en conf_patrones.json (Estrategia: Combinados).
    Argumento: historial_combinado_X es la secuencia del historial combinado.
    """
    # Asumo una longitud mínima de 7 para la búsqueda de patrones por consistencia
    if not historial_combinado_X or len(historial_combinado_X) < 7:
        return None

    # CORRECCIÓN: Invertir la secuencia.
    # El historial viene [Reciente, ..., Antiguo]. El JSON lo espera [Antiguo, ..., Reciente].
    historial_invertido = list(reversed(historial_combinado_X))
    patron_actual = "".join(historial_invertido) 
    ...
    # Buscar la coincidencia en la estrategia "Combinados"
    for estrategia in datos_patrones:
        if estrategia.get("estrategia") == "Combinados": 
            for patron_def in estrategia.get("Patrones", []):
                if patron_def.get("Patron") == patron_actual:
                    return patron_def 
